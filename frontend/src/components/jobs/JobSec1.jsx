  ...
  for (let i = 1; i <= totalPages; i++) {
    paginationLinks.push(
      <li key={i} className={i === currentPage ? "current" : ""}>
        <Link to={`/search/page/${i}`}>{i}</Link>
      </li>
    );
  }

  useEffect(() => {
    // Fetch data based on the current page using the `currentPage` variable
    // Construct the API URL with the `currentPage` parameter
    const apiUrl = `http://localhost:9000/api/helplines/show?pageNumber=${currentPage}`;

    fetch(apiUrl)
      .then((response) => response.json())
      .then((data) => {
        // Handle the data received from the API, update state if needed
      })
      .catch((error) => {
            ...
            <div className="content-tab">
              ...
              <TabPanel className="inner">
                ...
                <ul className="pagination-job padding">
                  ...
                  <li>
                    {currentPage > 1 && (
                      <Link to={`/search?pageNumber=${currentPage - 1}`}>
                        <i className="icon-keyboard_arrow_left"></i>
                      </Link>
                    )}
                  </li>
